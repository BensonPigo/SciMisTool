...
func InitGormDB(dbCfg DBConfig) (*gorm.DB, error) {
	...
	// 3. path 模式組 DSN（不帶 instanceName，也不重複 timeout）
	//    格式：sqlserver://user:pass@host/instance?database=DB&encrypt=…&connection+timeout=…
	dsn := fmt.Sprintf(
		"sqlserver://%s:%s@%s/%s"+
			"?database=%s&encrypt=%s&connection+timeout=%d",
		dbCfg.User, pwd,
		host, instance,
